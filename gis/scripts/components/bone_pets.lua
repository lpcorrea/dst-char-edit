...
function PetLeash:SpawnPetAt(x, y, z, prefaboverride, rebirth)
    ...
    if pet ~= nil then
		pet:SpawnChild("white_bone_small_puff")
        LinkPet(self, pet)
        ...
        if  self.inst.components.white_bone_cloak ~= nil and self.inst.components.white_bone_cloak:GetSkin() == "wb_armorgreed" then
            local timrleft = pet.components.timer:GetTimeLeft("timeover")
            pet.components.timer:SetTimeLeft("timeover", timrleft+480)
            print(pet.components.timer:GetTimeLeft("timeover"))
        end

		if rebirth and  pet.sg then
			pet.sg:GoToState("rebirth")
		end
...
function PetLeash:DespawnAllPets(timedone)
    local toremove = {}
    for k, v in pairs(self.pets) do
