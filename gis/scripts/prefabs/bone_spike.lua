local assets =
{
    Asset("ANIM", "anim/bone_spike.zip"),
}
...
local DAMAGE_RADIUS_PADDING = 0.6
...
local NEED_TAGS = { "_combat", "_health" }

local NON_NEED_TAGS = { "flying", "shadow", "ghost", "playerghost", "FX", "NOCLICK", "DECOR", "INLIMBO" }

local function DoDamage(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, 0, z, PHYSICS_RADIUS + DAMAGE_RADIUS_PADDING, NEED_TAGS, NON_NEED_TAGS)
...
local function StartSpike(inst, duration, variation)
    ...
    if variation > 1 then
        inst.AnimState:OverrideSymbol("bone1", "bone_spike", "bone"..tostring(variation))
    end

    inst.basefx = SpawnPrefab("bonespike_base")
...
local function fn()
    ...
    inst.AnimState:SetBuild("bone_spike")
    inst.AnimState:PlayAnimation("empty")
    inst.AnimState:SetFinalOffset(1)

    inst.Physics:SetMass(99999)
    inst.Physics:SetCollisionGroup(COLLISION.CHARACTERS)--(COLLISION.SMALLOBSTACLES)
    inst.Physics:ClearCollisionMask()
    inst.Physics:CollidesWith(COLLISION.ITEMS)
    inst.Physics:CollidesWith(COLLISION.CHARACTERS)
...
return Prefab("bonespike", fn, assets, prefabs),
    Prefab("bonespike_base", basefn, assets)
