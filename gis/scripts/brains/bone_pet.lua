...
local Bone_Pet_Brain = Class(Brain, function(self, inst)
            ...
            if addtltags ~= nil then
                for i, v in ipairs(addtltags) do
                    if target:HasTag(v) then
                        return BufferedAction(inst, target, action,weapon)
                    end
                end
            else
                return BufferedAction(inst, target, action,weapon)
...
function Bone_Pet_Brain:OnStart()
    ...
    local root = PriorityNode(
    {
        ...
        WhileNode(function() return ShouldDanceParty(self.inst) end, "Dance Party", --跳舞
            PriorityNode({
        ...
        WhileNode(function() return IsNearLeader(self.inst, KEEP_WORKING_DIST) end, "Leader In Range",
            PriorityNode({
                ...
                IfNode(function() return self.inst.components.combat.target ~= nil end, "Has Target",
                    PriorityNode({
                ...
                IfNode(function() return self.inst.components.inventory:EquipHasTag("CHOP_tool") end, "Keep Chopping",
                    DoAction(self.inst, function() return FindEntityToWorkAction(self.inst, ACTIONS.CHOP) end)),
                IfNode(function() return self.inst.components.inventory:EquipHasTag("MINE_tool") end, "Keep Mining",
                    DoAction(self.inst, function() return FindEntityToWorkAction(self.inst, ACTIONS.MINE) end)),
                IfNode(function() return self.inst.components.inventory:EquipHasTag("DIG_tool") end, "Keep Digging",
