...
local states =
{
    ...
    State{
        ...
        onenter = function(inst)
            inst.components.locomotor:RunForward()
            if not inst.AnimState:IsCurrentAnimation("run_werewilba_loop") then
    ...
    State{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
					if  inst.timedone then
						local fx = SpawnPrefab( "skeleton" )
						if fx then
							fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
							--fx.AnimState:PlayAnimation("idle6")
						end
					end
                    inst:Remove()
                end
...
return StateGraph("bone_pet", states, events, "idle", actionhandlers)
