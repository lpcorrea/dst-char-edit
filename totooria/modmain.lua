...
AddRecipe("tbook_birds", {GLOBAL.Ingredient("papyrus", 2), GLOBAL.Ingredient("bird_egg", 2)},
totooriatab, TECH.NONE, nil, nil, nil, nil, "totooria_builder", 
"images/inventoryimages.xml", "book_birds.tex" )

AddRecipe("tbook_gardening", {GLOBAL.Ingredient("papyrus", 2), GLOBAL.Ingredient("seeds", 1), GLOBAL.Ingredient("poop", 1)},
totooriatab, TECH.NONE, nil, nil, nil, nil, "totooria_builder", 
"images/inventoryimages.xml", "book_gardening.tex" )

AddRecipe("tbook_sleep", {GLOBAL.Ingredient("papyrus", 2), GLOBAL.Ingredient("nightmarefuel", 2)}, 
totooriatab, TECH.NONE, nil, nil, nil, nil, "totooria_builder", 
"images/inventoryimages.xml", "book_sleep.tex" )

AddRecipe("tbook_brimstone", {GLOBAL.Ingredient("papyrus", 2), GLOBAL.Ingredient("redgem", 1)}, 
totooriatab, TECH.NONE, nil, nil, nil, nil, "totooria_builder", 
"images/inventoryimages.xml", "book_brimstone.tex" )

AddRecipe("tbook_tentacles", {GLOBAL.Ingredient("papyrus", 2), GLOBAL.Ingredient("tentaclespots", 1)}, 
totooriatab, TECH.NONE, nil, nil, nil, nil, "totooria_builder", 
...
AddComponentPostInit("stewer", function(self)
    ...
    function self:Harvest(harvester)
    	if not harvester.components.totooriastatus or not harvester:HasTag("teji_dachu") then
...
AddComponentPostInit("crop", function(self)
	local oooldHarvest = self.Harvest
	function self:Harvest(harvester)
		if self.grower and self.grower.components.inspectable.nameoverride ~= "FARMPLOT" then
			return oooldHarvest(self, harvester)
		end
		
		local num = 1
		if harvester.components.totooriastatus and harvester:HasTag("teji_qiaoshou") then
			num = 2
			if self.matured or self.inst:HasTag("withered") then
		        ...
		        if self.grower ~= nil and
		            ...
		            product = GLOBAL.SpawnPrefab(temp.components.cookable ~= nil and temp.components.cookable.product or "seeds_cooked")
		            temp:Remove()
		        else
		        ...
		        if product ~= nil then
		            ...
		            if harvester ~= nil then
		            	if product.components.stackable ~= nil then
	                        product.components.stackable:SetStackSize(num)
	                    end
